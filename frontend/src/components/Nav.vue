<template>
  <nav class="navbar navbar-expand-md bg-light">
    ...
    <span> || </span>
    <router-link to="/maps" class="nav-link mx-3 fs-4">Maps</router-link>
    <span> || </span>
    <router-link to="/pictures" class="nav-link mx-3 fs-4">Pictures!</router-link>

    <button class="navbar-toggler d-flex justify-content-end text-warning m-3" type="button" data-bs-toggle="collapse" data-bs-target="#btn">
      <i class="bi bi-body-text">Tore</i>
    </button>

    <div class="collapse navbar-collapse" id="btn">
      <template v-if="this.$store.state.isAuthenticated">
        <div class="navbar-nav ms-auto d-flex align-items-center">
          <router-link to="myAccount" class="nav-item btn btn-primary me-2">My Account</router-link>
